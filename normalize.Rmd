...
```{r}
pheno_trans <- cbind(raw[,1],norm)
colnames(pheno_trans)[1] <- "IID"
pheno_trans[,"FID"] <- "LabExMI"
pheno_trans <- pheno_trans[,c(ncol(pheno_trans),1:(ncol(pheno_trans)-1))]
pheno_trans <- pheno_trans[-c(3)]
```

```{r}
pheno_trans = subset(pheno_trans, select = -c(N_HLADRposCD56hi.panel4, N_CD4negCD8neg_NKT_HLADRpos.panel3, N_FounderB.panel6, N_HLADRpos_in_CD4pos_EMRA.panel1) )
```

```{r}
write.table(pheno_trans, "./Data/phenotypes_linear.txt", sep = "\t", row.names=F, quote=F)
```
...
```{r}
bin <- cbind(pheno_trans[,c(1,2)], norm["N_FounderB.panel6"], norm["N_HLADRpos_in_CD4pos_EMRA.panel1"])
```

```{r}
bin$N_FounderB.panel6[bin$N_FounderB.panel6 > -0.9] <- 2
bin$N_FounderB.panel6[bin$N_FounderB.panel6 != 2] <- 1
```

```{r}
bin$N_HLADRpos_in_CD4pos_EMRA.panel1[bin$N_HLADRpos_in_CD4pos_EMRA.panel1 > 0.3] <- 2
bin$N_HLADRpos_in_CD4pos_EMRA.panel1[bin$N_HLADRpos_in_CD4pos_EMRA.panel1 != 2] <- 1
```

```{r}
write.table(bin, "./Data/phenotypes_logistic.txt", sep = "\t", row.names=F, quote=F)
```
